...
| `anim_fix_idles.py [-- --save --only Idle,Idle_02]` | the anim file | In-place fixes of Idle/Idle_02/Idle_03 (wrists forward, Idle grounded, Idle_02 straightened with Idle's arms); run once per action, it is not idempotent on Idle_02 | with `--save` |
